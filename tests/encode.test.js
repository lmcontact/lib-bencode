...
describe("getType", () => {
    ...
    it.each(cases)("result should be the bencoded corresponding type", elt => {
        expect(encode_1.getType(elt.value)).toBe(elt.type);
    });
});
describe("encodeString", () => {
    const cases = ["test", "bencoded", "a", "str", "abc:def", ":", ""].map(elt => ({
        str: elt,
        encoded: encode_1.encodeString(elt)
    }));
...
describe("encodeInt", () => {
    ...
    ].map(elt => ({ value: elt, encoded: encode_1.encodeInt(elt) }));
    ...
    it.each(cases)("result should contains exact bigint between 'i' and 'e'", elt => {
...
describe("encodeList", () => {
    ...
    it.each(cases1)("result should have 'l' as first character", elt => {
        const str = encode_1.encodeList(elt.value);
        expect(str[0]).toBe("l");
    });
    ...
    it.each(cases2)("should throw EncodeListError if list contains forbidden type", elt => {
        expect(() => {
            encode_1.encodeList(elt.value);
        }).toThrow(encode_1.EncodeError);
    });
});
describe("encodeDict", () => {
    const cases1 = [
    ...
    it.each(cases1)("result should have 'd' as first character", elt => {
        const str = encode_1.encodeDict(elt.value);
        expect(str[0]).toBe("d");
    });
    ...
    it.each(cases2)("should throw EncodedDictError if dict contains forbidden type", elt => {
        expect(() => {
            encode_1.encodeDict(elt);
        }).toThrow(encode_1.EncodeError);
    });
});
describe("encode", () => {
    ...
    it.each(cases1)("should returns the exact encoded value", elt => {
        const str = encode_1.encode(elt.value);
        expect(str).toBe(elt.encoded);
    });
    const cases2 = [
        { value: null },
    ...
    it.each(cases2)("should throw an EncodeError if arg is of a forbidden type", elt => {
        expect(() => {
            encode_1.encode(elt.value);
        }).toThrow(encode_1.EncodeError);
    });
});
